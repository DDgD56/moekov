...
function drawStation(s, x, y){
  ...
  else if(s.panel==='shop'){
    // 🛒 떠돌이 상인 — 손수레 좌판 + 밀짚모자 상인
    pOval(0, 22, 24, 5, 'rgba(0,0,0,.3)');
    // 수레 바퀴
    pBlob(-16, 18, 6, '#3a2c1c'); pBlob(-16, 18, 3, '#5a4a30');
    pBlob(16, 18, 6, '#3a2c1c'); pBlob(16, 18, 3, '#5a4a30');
    // 좌판
    pRect(-24, 2, 48, 14, '#5a3e28');
    pRect(-22, 4, 44, 10, '#8a6238');
    // 진열 상품 (알록달록)
    pRect(-18, -2, 7, 6, '#c94a3a');
    pRect(-8, -3, 7, 7, '#4a8ac0');
    pRect(2, -2, 7, 6, '#e8c84a');
    pRect(12, -3, 6, 7, '#6aa04a');
    // 차양 기둥 + 줄무늬 차양
    pRect(-24, -26, 3, 28, '#3a2c1c');
    pRect(21, -26, 3, 28, '#3a2c1c');
    pRect(-28, -32, 56, 8, '#b8524a');
    for(let i=0;i<4;i++) pRect(-24+i*14, -32, 7, 8, '#e8dcc0');
    pRect(-28, -25, 56, 2, '#7a3a30');
    // 상인 (밀짚모자 미니, 좌판 뒤)
    pRect(-6, -20, 12, 10, '#e8d0a8');   // 얼굴
    pRect(-3, -16, 2, 2, '#1a1410');     // 눈
    pRect(2, -16, 2, 2, '#1a1410');
    pRect(-10, -24, 20, 4, '#d8b85a');   // 밀짚모자 챙
    pRect(-6, -28, 12, 5, '#e8cc6a');    // 모자 위
    pRect(-7, -10, 14, 8, '#7a5a8a');    // 몸통(보라 옷)
  }
