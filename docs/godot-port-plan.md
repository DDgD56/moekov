...
- [x] 🛒 떠돌이 상인 — 일일 재고 6칸(부착물3+몸통+장비+음식)·유료 리롤, 웹 상점 규칙 1:1 (2026-07-16)
- [x] 사격장 — 과녁 3개·DPS(3초 창)·명중률·장착 총 실사격, 케이브 문 스테이션 (2026-07-16)
...
**남은 것:** 전부 완료 — 집/실내 · 지형 다양화(강/다리·농장·야적장·차량) · 적 팔레트 틴트 · 미식별 조사 ·
**웹 격차 해소 (2026-07-16 완결):** 레이드 이변 5종+희귀/엑조틱/유물 롤 · 다중 탈출구(2~3) · 공장 건물 · 스나이퍼 · 총 보관대 UI · Ctrl클릭 · 온보딩 · 보스 드롭 테이블 · 황금미니 · 판 요약+누적 기록 · 시체 컨테이너/소멸 규칙 · 야간 원뿔 시야 · 바리케이드·기름웅덩이 · 엑조틱 기믹 4종(chain/boom/lifesteal/ricochet) · **유기적 블롭 지형+숲 외곽 · 호수/섬 요충지 · 대저택 2축 그리드** → 웹 buildRaid 파리티 완성. 잔여: 음원(습지/케이브 BGM) 에셋뿐

**시야 설계 규칙 (2026-07-15 확정)**
- 전투 중 상자/오브젝트를 열어 보는 동안 **배경(월드)이 보이면 안 됨** — 루팅은 위험한 순간이어야 한다 (패널 뒤 완전 암전)
- 집 안에 들어가면 **바깥이 보이면 안 됨** — 입구(문) 방향만 시야각에 따라 살짝 보이는 정도
...
**작업**
- [x] 오디오 — SFX 12종 런타임 파형 합성(에셋 0) + 지역 BGM 3매핑 (2026-07-15)
- [x] HUD·토스트·옵션 — 케이브 O키 옵션 패널: BGM/SFX 볼륨 ±dB(user://config.json), 전체화면 전환 (2026-07-16)
- [x] 성능 프로파일 — 적 90 · 탄 240 · 숫자 60 · 파티클 220 상한 (2026-07-16)
- [x] Windows export 스모크 — PE32+ x86-64 단일 exe(pck 내장, 117MB), 버전 0.6.0.0 표기. 남은 것: 아이콘(rcedit)·클린 Windows 실기기 체크 (2026-07-16)
- [ ] (선택) 설치 패키지 / zip 배포
- [ ] 스모크 체크리스트 (아래 §7) — 클린 Windows 실기기 필요
